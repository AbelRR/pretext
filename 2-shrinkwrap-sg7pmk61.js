...
// pages/demos/examples/shrinkwrap.ts
var conversations = [
...
var prepared = conversations.map((c) => prepareWithSegments(c.text, font));
function shrinkwrapWidth(prep, maxWidth) {
  const textMax = maxWidth - hPad;
  if (textMax <= 0)
    return maxWidth;
  let baseLineCount = 0;
  walkLineRanges(prep, textMax, () => {
    baseLineCount++;
  });
  if (baseLineCount <= 1) {
    let w = 0;
    walkLineRanges(prep, textMax, (line) => {
      w = line.width;
    });
...
function render(maxBubbleWidth) {
...
var valLabel = document.getElementById("width-val");
