...
// pages/demos/examples/infinite-scroll.ts
...
var containerWidth = Math.min(600, window.innerWidth) - 64;
var prepared = messages.map((m) => prepare(m, font));
var heightCache = new Float32Array(TOTAL_ITEMS);
var offsets = new Float64Array(TOTAL_ITEMS + 1);
offsets[0] = 0;
for (let i = 0;i < TOTAL_ITEMS; i++) {
...
var totalHeight = offsets[TOTAL_ITEMS];
var computeTime = performance.now() - startTime;
...
function renderVisible() {
  ...
  while (lo < hi) {
    const mid = lo + hi >>> 1;
    if (offsets[mid + 1] < scrollTop - BUFFER)
      lo = mid + 1;
    else
      hi = mid;
  }
