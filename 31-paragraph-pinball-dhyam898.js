...
// pages/demos/examples/paragraph-pinball.ts
...
var colors = ["#ff6b6b", "#ffd93d", "#6bcb77", "#4d96ff", "#cc5de8", "#ff922b", "#20c997", "#38bdf8"];
var font = "bold 12px 'Helvetica Neue', Helvetica, Arial, sans-serif";
var lineHeight = 16;
await document.fonts.ready;
var bumpers = bumperTexts.map((text, i) => {
  const w = 100 + Math.random() * 60;
  const prepared = prepareWithSegments(text, font);
  const result = layout(prepared, w - 16, lineHeight);
...
var ball = { x: W / 2, y: H - 60, vx: 0, vy: 0, r: 8, active: false };
var score = 0;
...
function update() {
  ...
  for (const b of bumpers) {
    ...
    if (dist < ball.r) {
      ...
      const result = layout(b.prepared, b.currentWidth - 16, lineHeight);
      b.w = b.currentWidth;
