...
// pages/demos/examples/text-maze.ts
...
var font = "11px 'Helvetica Neue', Helvetica, Arial, sans-serif";
var lineHeight = 14;
var colors = ["#334155", "#1e3a5f", "#312e81", "#4c1d95", "#831843", "#7f1d1d"];
  ...
  for (let c = 0;c < cols; c++) {
    if (Math.random() > 0.45 && !(r === 1 && c === 1)) {
      ...
      walls.push({
        ...
        y: r * cellSize + 2,
        w,
        h: Math.max(cellSize - 4, result.height + 8),
        text,
...
var playerSize = 12;
var speed = 3;
var goalX = (cols - 2) * cellSize + cellSize / 2;
var goalY = (rows - 2) * cellSize + cellSize / 2;
var keys = {};
...
function collides(x, y) {
  for (const w of walls) {
    if (x + playerSize > w.x && x - playerSize < w.x + w.w && y + playerSize > w.y && y - playerSize < w.y + w.h)
      return true;
  }
  return false;
}
