...
// pages/demos/examples/height-prediction.ts
var messages = [
...
var prepared = messages.map((msg) => prepare(msg, font));
function renderPanel(viewportId, statsId, getHeight) {
  const viewport = document.getElementById(viewportId);
  const statsEl = document.getElementById(statsId);
  const actualHeights = [];
  const predictedHeights = [];
