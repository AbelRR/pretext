...
// pages/demos/examples/dvd-screensaver.ts
var canvas = document.getElementById("canvas");
var ctx = canvas.getContext("2d");
var cornerCount = document.getElementById("corner-count");
...
var title = "How to Measure Text";
var subtitle = "March 2026";
var essay = `The web has a dirty secret: it can't measure text without rendering it. Every time you call getBoundingClientRect or read offsetHeight, the browser must lay out the entire document. This is called a "reflow," and it is one of the most expensive operations a browser performs.
...
var titleFont = "bold 20px 'Georgia', 'Times New Roman', serif";
var subtitleFont = "14px 'Georgia', 'Times New Roman', serif";
var bodyFontCanvas = "15px 'Georgia', 'Times New Roman', serif";
var lineHeight = 22;
var essayWidth = 380;
var padding = 28;
...
var preparedTitle = prepareWithSegments(title, titleFont);
var preparedSubtitle = prepareWithSegments(subtitle, subtitleFont);
var preparedBody = prepareWithSegments(essay, bodyFontCanvas);
var titleResult = layoutWithLines(preparedTitle, essayWidth, titleLineHeight);
var subtitleResult = layoutWithLines(preparedSubtitle, essayWidth, subtitleLineHeight);
var bodyResult = layoutWithLines(preparedBody, essayWidth, lineHeight);
var blockWidth = essayWidth + padding * 2;
var blockHeight = padding + titleResult.height + 8 + subtitleResult.height + 16 + bodyResult.height + padding;
var colors = [
  "#e8e0d0",
  "#ffd4a3",
  "#a3d9ff",
...
var x = Math.random() * Math.max(1, W - blockWidth);
var y = Math.random() * Math.max(1, H - blockHeight);
var vx = 1.2;
var vy = 0.8;
var color = colors[0];
var corners = 0;
...
function update() {
  ...
  if (y <= 0) {
    ...
    vy = Math.abs(vy);
    hitY = true;
  } else if (y + blockHeight >= H) {
    y = H - blockHeight;
    vy = -Math.abs(vy);
    hitY = true;
  }
  if (hitX || hitY)
    color = pickNewColor(color);
  if (hitX && hitY) {
    corners++;
