...
// pages/demos/examples/text-tetris.ts
var canvas = document.getElementById("canvas");
var ctx = canvas.getContext("2d");
var scoreEl = document.getElementById("score");
var linesClearedEl = document.getElementById("lines-cleared");
...
var colors = ["#ff6b6b", "#ffd93d", "#6bcb77", "#4d96ff", "#cc5de8", "#ff922b", "#20c997"];
var font = "bold 11px 'Helvetica Neue', Helvetica, Arial, sans-serif";
...
var lastTime = 0;
var gameOver = false;
function canPlace(col, row, span) {
  if (col < 0 || col + span > COLS || row >= ROWS)
    return false;
  if (row < 0)
    return true;
...
document.addEventListener("keydown", (e) => {
  if (!falling || gameOver)
    return;
  if (e.key === "ArrowLeft") {
    if (canPlace(falling.col - 1, falling.row, falling.block.colSpan))
      falling.col--;
  } else if (e.key === "ArrowRight") {
    if (canPlace(falling.col + 1, falling.row, falling.block.colSpan))
      falling.col++;
  } else if (e.key === "ArrowDown") {
    while (canPlace(falling.col, falling.row + 1, falling.block.colSpan))
      falling.row++;
    placeFalling();
    score += 10;
    scoreEl.textContent = String(score);
