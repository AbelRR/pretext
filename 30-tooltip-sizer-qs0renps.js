...
// pages/demos/examples/tooltip-sizer.ts
...
var tipFont = "13px 'Helvetica Neue', Helvetica, Arial, sans-serif";
var lineHeight = 18;
var tipMaxWidth = 260;
var tipPad = 10;
var colors = ["#ff6b6b", "#ffd93d", "#6bcb77", "#4d96ff", "#cc5de8", "#ff922b", "#20c997", "#38bdf8"];
await document.fonts.ready;
var dots = tooltips.map((t, i) => {
  const cols = 4;
  const rows = 2;
  const col = i % cols;
  ...
  let baseLines = 0;
  walkLineRanges(prepared, textWidth, () => {
    baseLines++;
  });
  ...
  if (baseLines > 1) {
    ...
    let w = 0;
    walkLineRanges(prepared, textWidth, (line) => {
      w = line.width;
    });
...
function draw() {
  ...
  if (hoveredDot) {
    ...
    if (tx + d.tipWidth > W - 10)
      tx = d.x - d.tipWidth - 20;
    if (ty < 10)
      ty = d.y + 30;
