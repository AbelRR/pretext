...
// pages/demos/examples/balanced-text.ts
var headlines = [
...
var font = "18px 'Helvetica Neue', Helvetica, Arial, sans-serif";
var maxWidth = 420;
var padding = 32;
await document.fonts.ready;
var unbalancedEl = document.getElementById("unbalanced");
var balancedEl = document.getElementById("balanced");
headlines.forEach((text) => {
  const prep = prepareWithSegments(text, font);
  const textWidth = maxWidth - padding;
  let baseLineCount = 0;
  walkLineRanges(prep, textWidth, () => {
    baseLineCount++;
  });
  ...
  walkLineRanges(prep, lo, (line) => {
    if (line.width > balancedWidest)
      balancedWidest = line.width;
  });
