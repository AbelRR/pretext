...
// pages/demos/examples/breathing-text.ts
...
var text = `In the beginning was the word, and the word needed to be measured. For decades, the web's answer was to render text into the DOM and ask the browser how tall it was. This worked, but at a terrible cost: every measurement forced a synchronous reflow — the browser had to lay out the entire document just to answer a simple question about one paragraph. Pretext changes this. By measuring text segments on canvas and caching the results, it reduces layout to pure arithmetic. The prepare step runs once. After that, computing height at any width costs about 0.0002 milliseconds. That's fast enough to run hundreds of times per frame, smooth enough for animation, cheap enough to make text layout feel like it was never the bottleneck it always was.`;
var font = "18px 'Georgia', 'Times New Roman', serif";
var lineHeight = 28;
