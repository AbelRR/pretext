...
// pages/demos/examples/sticky-notes.ts
...
var noteColors = ["#fff9c4", "#f8bbd0", "#c8e6c9", "#bbdefb", "#ffe0b2", "#e1bee7", "#b2dfdb", "#ffccbc"];
...
function createNote(x, y, text) {
  ...
  return {
    ...
    prepared,
    lines: result.lines,
    height: result.height + pad * 2 + 16
  };
}
var notes = defaultTexts.map((t, i) => {
...
function noteAt(mx, my) {
  for (let i = notes.length - 1;i >= 0; i--) {
    const n = notes[i];
    if (mx >= n.x && mx <= n.x + n.w && my >= n.y && my <= n.y + n.height)
      return n;
  }
  return null;
}
